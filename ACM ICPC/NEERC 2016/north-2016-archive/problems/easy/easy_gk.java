            ...
            loop: for (Cell cell : cells) {
                if (paper.add(cell)) {
                    ...
                    if (image.rows.size() == paper.rows.size() && ((image.hash * X_POWERS[dx] % MODULO) * Y_POWERS[dy]) % MODULO == paper.hash) {
                        final Iterator<Cell> ii = image.rows.keySet().iterator();
                        final Iterator<Cell> pi = paper.rows.keySet().iterator();
                        while (ii.hasNext()) {
                            final Cell ic = ii.next();
                            final Cell pc = pi.next();
                            if (pc.x != ic.x + dx || pc.y != ic.y + dy) {
                                continue loop;
                            }
                        }
                        out.println("YES");
