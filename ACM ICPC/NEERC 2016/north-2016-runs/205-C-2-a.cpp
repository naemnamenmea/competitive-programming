...
int main()
{
	...
	for (int i = 0; i < n; i++){
		num = a[i].second;
		val = a[i].first;
		it = upper_bound(a.begin(), a.end(), make_pair(val, n))-a.begin();
		//cout<<it<<" "<<cntA[val]<<endl;
		ansA[num] = max(it - cntA[val] ,0);
		ans[num] += ansA[num];
		num = b[i].second;
		val = b[i].first;
		it = upper_bound(b.begin(), b.end(), make_pair(val, n)) - b.begin();
		ansB[num] =max( it - cntB[val] ,0);
		ans[num] += ansB[num];
	}
