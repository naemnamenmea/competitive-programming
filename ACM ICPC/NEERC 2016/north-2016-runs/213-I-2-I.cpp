...
pair<int, int> a[2][2][2];
pair<int, int> a_new[2][2][2];
...
//			cerr << i << " " << "here!" << endl;
			for (int k = 0; k < 2; k++) {
				for (int j = 0; j < 2; j++) {
					swap(a[k][j][0], a[k][j][1]);
				}
			}
		}
				
		if (a[dx][dy][0].first == -1) {
//			cerr << "fair_count " << i << endl;
			ans[i] = fair_count(i, n);
		} else {
			pair<int, int> t = a[dx][dy][0];
//			cerr << t.second << endl;
			if ((t.second) %2 == 0)
				ans[i] = ans[t.first];
        	else {
...
//        	cerr << i << " " << ans[i] << " " << t.first << " " << ans[t.first] << endl;    
		...
		for (int i = 0; i < 2; i++) {
	    	for (int j = 0; j < 2; j++) {
	    		a_new[i][j][0] = a[i ^ dx][j ^ dy][0];
	    		a_new[i][j][1] = a[i ^ dx][j ^ dy][1];
	    	}
		}
		if (dx == 1 && dy == 1) {
		a_new[dx][dy][1] = make_pair(i - 1, 1);
		} else {
		a_new[dx][dy][0] = make_pair(i - 1, 0);
		}
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				a[i][j][0] = a_new[i][j][0];
